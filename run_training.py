...
def main():
    ...
    args.output_dir = os.path.join(args.output_dir, args.model_type, args.task_name, "train")
    ...
    if args.do_train:
        ...
        # NOTE: Cache vocab_size here since it may be modified.
        ...
        # NOTE: ``config.vocab_size'' has also been modified scerectly while resizing the embeddings,
        ...
        if not os.path.exists(args.init_model_path):
            tokenizer.save_pretrained(args.init_model_path)
            config.save_pretrained(args.init_model_path)
            model.save_pretrained(args.init_model_path)
            ...
            for step, batch in enumerate(train_loader):
                ...
                if step % args.num_grad_accum_steps == 0 or step == len(train_loader) - 1:
                    if is_fp16:
                        ...
                        torch.lnn.utils.clip_grad_norm_(model.parameters(), args.max_grad_norm)
