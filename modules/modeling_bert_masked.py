...
from transformers.models.bert.modeling_bert import BertModel, BertEncoder, BertLayer, BertOutput, BertAttention, BertSelfAttention, BertIntermediate
...
class GradBasedMaskedBertIntermediate(BertIntermediate):
   def forward(self, hidden_states):
        hidden_states = self.dense(hidden_states)
        hidden_states_ = self.intermediate_act_fn(hidden_states)
        return hidden_states, hidden_states_
...
class GradBasedMaskedBertLayer(BertLayer):
    def __init__(self, config):
        ...
        self.intermediate = GradBasedMaskedBertIntermediate(config)
        self.intermediate_layer_output = None
        self.mask_ffn = False
    ...
    def forward(
        ...
        if self.mask_ffn:
            ...
            self.intermediate_layer_output, intermediate_output = self.intermediate(attention_output)
